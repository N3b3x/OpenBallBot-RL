...
class RGBDInputs:
    ...
    def __init__(self, mjc_model, height, width, cams: List[str],
                 disable_rgb: bool):
    ...
    def __call__(self, data, cam_name: str):
        ...
        depth[
            depth >=
            1.0] = 1.0  #with robot orientations - especially near failure - the depth cameras might catch a glimpse of the sky which will return large values
...
class BBotSimulation(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Dict({
            ...
            "angular_vel":
            gym.spaces.Box(low=-2, high=2, shape=(3, ), dtype=_default_dtype),
            "vel":
            gym.spaces.Box(low=-2, high=2, shape=(3, ), dtype=_default_dtype),
            "motor_state":
            gym.spaces.Box(-2.0, 2.0, shape=(3, ), dtype=_default_dtype),
            "actions":
            gym.spaces.Box(-1.0, 1.0, shape=(3, ), dtype=_default_dtype),
            "rgbd_0":
            ...
            gym.spaces.Box(low=0.0,
                           ...
                           shape=(num_channels, im_shape["h"], im_shape["w"]),
                           dtype=_default_dtype),
            "relative_image_timestamp":
            gym.spaces.Box(
                low=0.0, high=0.1, shape=(1, ), dtype=_default_dtype
            ),  #the lag between high-frequencey proprio and low-freq cams
        ...
        if not disable_cameras:
            self.rgbd_inputs = RGBDInputs(self.model,
                                          height=im_shape["h"],
                                          width=im_shape["w"],
                                          cams=["cam_0", "cam_1"],
        ...
        self.max_abs_obs = {
            x: -1
            for x in ["orientation", "angular_vel", "vel", "pos"]
        }
    ...
    def _reset_terrain(self):
        ...
        assert self.model.hfield_size[0, 0] == self.model.hfield_size[
            0,
            1], f"terrain should have equal length and width (got {self.model.hfield_size[0,:2]} in xml file)"
        ...
        if self.passive_viewer is not None:
            self.passive_viewer.update_hfield(
                mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_HFIELD,
                                  "terrain"))
        ...
        ball_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM,
                                    "the_ball")
        assert self.data.geom_xpos[ball_id][0] == 0 and self.data.geom_xpos[
            ball_id][
                1] == 0, "terrain generation assumes that the ball is centered at (0,0)"
        ball_pos = self.data.geom_xpos[ball_id]
        ball_size = self.model.geom_size[ball_id]
        aabb_min = ball_pos - ball_size[
            0]  #note that ball_size is specified with only one element in the xml
        ...
        sub_terr = terrain_mat[center_idx -
                               abs(int(aabb_min[0] // cell_size)):center_idx +
                               int(aabb_max[0] // cell_size) + 1, center_idx -
                               abs(int(aabb_min[1] // cell_size)):center_idx +
                               int(aabb_max[1] // cell_size) + 1]
    ...
    def reset(self, seed=None, goal: str = "random", **kwargs):
        ...
        self.rgbd_inputs.reset(
            self.model)  #otherwise it wont get updated with the new hfield
        self.prev_im_pair = StampedImPair(im_0=None, im_1=None, ts=0)
        ...
        if self.log_dir is None and not self.eval_env:
            rand_str = ''.join(
                self._np_random.permutation(
                    list(string.ascii_letters + string.digits))[:12])
    ...
    def _save_logs(self):
                ...
                for ii in range(len(self.rgbd_hist_0)):
                    if not self.depth_only:
                        cv2.imwrite(
                            ...
                            (cv2.merge(
                                cv2.split(
                                    self.rgbd_hist_0[ii][:, :, :3])[::-1]) *
                             255).astype("uint8"))
                        cv2.imwrite(
                            f"{dir_name_rgb}/rbgd_b_{ii}.png",
                            (cv2.merge(
                                cv2.split(
                                    self.rgbd_hist_1[ii][:, :, :3])[::-1]) *
                             255).astype("uint8"))
                        cv2.imwrite(f"{dir_name_d}/depth_a_{ii}.png",
                                    (self.rgbd_hist_0[ii][:, :, 3] *
                                     255).astype("uint8"))
                        cv2.imwrite(f"{dir_name_d}/depth_b_{ii}.png",
                                    (self.rgbd_hist_1[ii][:, :, 3] *
                                     255).astype("uint8"))
                    else:
                        cv2.imwrite(f"{dir_name_d}/depth_a_{ii}.png",
                                    (self.rgbd_hist_0[ii] *
        ...
        if self.log_options["reward_terms"]:
            if len(self.reward_term_1_hist):
                np.save(self.log_dir + "/term_1",
                        np.array(self.reward_term_1_hist))
                np.save(self.log_dir + "/term_2",
    ...
    def _get_obs(self, last_ctrl):
        ...
        if not self.disable_cameras:
            ...
            if self.prev_im_pair.im_0 is None or delta_time >= 1.0 / self.camera_frame_rate:
                rgbd_0 = self.rgbd_inputs(self.data,
                                          "cam_0").astype(_default_dtype)
                rgbd_1 = self.rgbd_inputs(self.data,
                                          "cam_1").astype(_default_dtype)
                if self.log_options["cams"]:
        ...
        rot_vec = quaternion.as_rotation_vector(orientation).astype(
            _default_dtype)
        ...
        ) / self.opt_timestep if self.prev_pos is not None else np.zeros_like(
            position)
        ...
        if self.prev_orientation is not None:
            ...
            angular_vel = np.clip(vee(W) / self.opt_timestep,
                                  a_min=-2.0,
                                  a_max=2.0).astype(_default_dtype)
        else:
        ...
        if self.disable_cameras:
            ...
            obs = {
                ...
                last_ctrl,
                "rgbd_0":
                rgbd_0.transpose(2, 0, 1),
                "rgbd_1":
                rgbd_1.transpose(2, 0, 1),
                "relative_image_timestamp":
                np.array([self.data.time - self.prev_im_pair.ts
                          ]).astype(_default_dtype),
            }
    ...
    def _get_info(self):
        ...
        position = self.data.xpos[self.model.body("base").id].copy().astype(
            _default_dtype)
        info = {
            "success": False,
            "failure": False,
    ...
    def step(self, omniwheel_commands):
        ...
        ):  #to remove some benign (and very rare) warnings regarding objective convexity
            mujoco.mj_step(self.model, self.data)

        self.data.xfrc_applied[self.model.body("base").id, :3] = np.zeros(
            3
        )  #this is to reset the initial force that is applied. From the documentation,
        ...
        reward = self.reward_obj(
            obs) / 100  #regularization and survival are added later

        self.reward_term_1_hist.append(reward)

        action_regularization = -0.0001 * (np.linalg.norm(omniwheel_commands)**
                                           2)
        ...
        if self.passive_viewer:
            ...
            with self.passive_viewer.lock():
                ...
                if 0:
                    ...
                    hh = 0.5
                    mujoco.mjv_initGeom(self.passive_viewer.user_scn.geoms[
                    ...
                    mujoco.mjv_connector(self.passive_viewer.user_scn.geoms[
                        self.passive_viewer.user_scn.ngeom - 1],
        ...
        if self.step_counter >= self.max_ep_steps:
            if self.verbose:
                print(
                    f"terminated. Cause: {self.step_counter}>self.max_ep_steps"
                )
            terminated = True

        #compute angle with upright vector
        gravity = np.array([0, 0, -1.0]).astype(_default_dtype).reshape(3, 1)
        gravity_local = (quaternion.as_rotation_matrix(
            quaternion.from_rotation_vector(obs["orientation"][-3:])).T
                         @ (gravity)).reshape(3)
        ...
        if angle_in_degrees > max_allowed_tilt:
            ...
            if self.verbose:
                print(
                    f"failure after {self.step_counter}. Reason: tilt_angle > {max_allowed_tilt} "
                )
        ...
        if terminated:
            if self.verbose:
                print(
                    colored(
                        f"G_tau=={self.G_tau}, num_steps=={self.step_counter}",
                        "magenta",
                        attrs=["bold"]))
            self._save_logs()
