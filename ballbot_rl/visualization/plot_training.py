...
def plot_train_val_progress(csv_file, eval_only):
    ...
    with open(csv_file, newline='') as csvfile:
        ...
        line_count = sum(1
                         for _ in csv.DictReader(csvfile, fieldnames=headers))
        if line_count < 3:
            msg = colored("not enough lines in the csv file yet",
                          "red",
                          attrs=["bold"])
            raise Exception(msg)
        csvfile.seek(0)
        ...
        #plot rewards
        if not eval_only:
            plt.plot(progress_data["rollout_domain"],
                     progress_data["rollout/ep_rew_mean"],
                     linewidth=_linewidth,
                     label="train",
                     color="blue")
        plt.plot(progress_data["eval_domain"],
                 progress_data["eval/mean_reward"],
                 linewidth=_linewidth,
                 label="eval",
                 color="darkorange")
        ...
        plt.ylabel(f'Mean reward \n(average over last N environments)',
                   fontsize=_fontsize_labels)
        ...
        if not eval_only:
            plt.plot(progress_data["rollout_domain"],
                     progress_data["rollout/mean_ep_length"],
                     linewidth=_linewidth,
                     label="train",
                     color="blue")
        plt.plot(progress_data["eval_domain"],
                 progress_data["eval/mean_ep_length"],
                 linewidth=_linewidth,
                 label="eval",
                 color="darkorange")
        #plt.xlabel('Environment timesteps (millions)', fontsize=_fontsize_labels)
        plt.xlabel('Environment timesteps', fontsize=_fontsize_labels)
        plt.ylabel(f'Average episode length \n(over last N environments)',
                   fontsize=_fontsize_labels)
        plt.tick_params(axis='both', labelsize=_fontsize_ticks)
...
def plot_loss_evolutions(csv_file, config_file):
    ...
    plt.plot([x for x in progress_data["train/entropy_loss"]],
             "r",
             label="entropy")
    ...
    plt.plot([x for x in progress_data["train/value_loss"]],
             "b",
             label="value loss")
    plt.legend(fontsize=18)
    plt.legend(fontsize=18)
